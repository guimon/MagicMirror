...
// disabling timezone for periodic events as it's broken https://github.com/jkbrzt/rrule/issues/355
rule.origOptions.tzid = null;
rule.options.tzid = null;
					...
					if (CalendarFetcherUtils.isFullDayEvent(event)) {
						...
						if (config.includePastEvents) {
							...
							pastLocal = pastMoment.utc().toDate();
						} else {
							// otherwise use NOW.. cause we shouldn't use any before now
							pastLocal = moment().utc().toDate(); //now
						}
						futureLocal = futureMoment.utc().toDate(); // future
					}
